...
export async function chatWithGemini(
  ...
  if (API_KEYS.length === 0) {
    console.error('Gemini APIキーが設定されていません。GEMINI_API_KEYS環境変数を確認してください。');
    throw new Error('Gemini APIキーが設定されていません');
  }

  console.log(`Gemini API keys loaded: ${API_KEYS.length}個`);
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    console.log(`Gemini attempt ${attempt + 1}/${maxRetries}, key index: ${currentKeyIndex}`);
    ...
    try {
      ...
      console.error(`Gemini key index ${currentKeyIndex} failed:`, errorMsg);
